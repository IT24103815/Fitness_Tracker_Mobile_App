...
const CreateChallengeScreen = ({ navigation }) => {
  ...
  const [tasks, setTasks] = useState([{ exercise: null, sets: '3', reps: '10' }]);
  const [rewardStats, setRewardStats] = useState({
    strength: '5', endurance: '0', stamina: '0', flexibility: '0', explosivePower: '0', coreStability: '0'
  });
  ...
  const handleCreate = async () => {
    ...
    if (tasks.some(t => !t.exercise)) {
      Alert.alert('Incomplete Tasks', 'Please select an exercise for each slot');
      return;
    }
    ...
    if (isNaN(xpReward) || xpReward < 0) {
      newErrors.rewardXP = 'Invalid XP value';
    } else if (xpReward > 800) {
      newErrors.rewardXP = 'XP reward cannot exceed 800 XP';
    }

    const invalidStats = Object.entries(rewardStats).filter(([stat, value]) => parseInt(value) > 20);
    if (invalidStats.length > 0) {
      newErrors.rewardStats = 'Individual stat rewards cannot exceed 20 points';
    }
    ...
    try {
      ...
      Alert.alert('Error', err.response?.data?.message || 'Failed to create challenge');
    } finally {
      setLoading(false);
    }
  };
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <TextInput 
          ...
          placeholder="e.g. Spartan Strength Week"
        />
        ...
        <View style={styles.row}>
          ...
          <View style={{flex: 1, marginLeft: 10}}>
            ...
            <TextInput 
              style={[styles.input, errors.rewardXP && styles.inputError]} 
              value={form.rewardXP} 
              keyboardType="numeric" 
              onChangeText={v => {
                setForm({...form, rewardXP: v});
                if (errors.rewardXP) setErrors({...errors, rewardXP: null});
              }}
            />
            {errors.rewardXP && <Text style={styles.errorText}>{errors.rewardXP}</Text>}
          </View>
        </View>
      </View>
        ...
        <View key={idx} style={styles.taskCard}>
          ...
          >
            <Text style={{color: task.exercise ? '#fff' : '#94A3B8'}}>
              {task.exercise ? task.exercise.name : 'Choose Exercise from Library...'}
            </Text>
          </TouchableOpacity>

          <View style={styles.row}>
            <TextInput style={[styles.input, {flex: 1}]} placeholder="Sets" keyboardType="numeric" value={task.sets} onChangeText={v => {
              const next = [...tasks];
              next[idx].sets = v;
              setTasks(next);
            }}/>
            <TextInput style={[styles.input, {flex: 2, marginLeft: 10}]} placeholder="Reps (e.g. 10 or 30s)" value={task.reps} onChangeText={v => {
              const next = [...tasks];
              next[idx].reps = v;
              setTasks(next);
            }}/>
      ...
      <View style={[styles.card, errors.rewardStats && styles.inputError]}>
        <View style={styles.statGrid}>
          {Object.keys(rewardStats).map(stat => (
            <View key={stat} style={styles.statInputGroup}>
              ...
              <TextInput 
                style={[styles.input, parseInt(rewardStats[stat]) > 20 && styles.inputError]} 
                keyboardType="numeric" 
                value={rewardStats[stat]} 
                onChangeText={v => {
                  setRewardStats({...rewardStats, [stat]: v});
                  if (errors.rewardStats) setErrors({...errors, rewardStats: null});
                }}
              />
            </View>
          ))}
        </View>
        {errors.rewardStats && <Text style={[styles.errorText, {marginTop: 10}]}>{errors.rewardStats}</Text>}
      </View>

      <TouchableOpacity style={styles.submitBtn} onPress={handleCreate} disabled={loading}>
        {loading ? <ActivityIndicator color="#fff"/> : <Text style={styles.submitBtnText}>Create Challenge Template</Text>}
      </TouchableOpacity>
      ...
      <Modal animationType="slide" transparent={true} visible={modalVisible} onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {libraryLoading ? <ActivityIndicator color="#3B82F6"/> : (
              <FlatList
                data={exercises}
...
const styles = StyleSheet.create({
  ...
  exercisePicker: { backgroundColor: '#0F172A', borderDimWidth: 1, padding: 16, borderRadius: 12, marginBottom: 10, borderStyle: 'dotted', borderWidth: 1, borderColor: '#334155' },
