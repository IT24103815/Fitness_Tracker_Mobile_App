...
const EditChallengeScreen = ({ route, navigation }) => {
  ...
  const [form, setForm] = useState({
    ...
    category: challenge.category || ['strength'],
    difficulty: challenge.difficulty,
    minLevel: challenge.minLevel?.toString() || '1',
    rewardXP: challenge.rewardXP?.toString() || '200'
  });

  const [tasks, setTasks] = useState(challenge.tasks || [{ exercise: null, sets: '3', reps: '10' }]);
  const [rewardStats, setRewardStats] = useState({
    strength: challenge.rewardStats?.strength?.toString() || '0',
    endurance: challenge.rewardStats?.endurance?.toString() || '0',
    stamina: challenge.rewardStats?.stamina?.toString() || '0',
    flexibility: challenge.rewardStats?.flexibility?.toString() || '0',
    explosivePower: challenge.rewardStats?.explosivePower?.toString() || '0',
    coreStability: challenge.rewardStats?.coreStability?.toString() || '0',
  });
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/challenges/${challenge._id}`, payload);
      Alert.alert('Success', 'Challenge updated!');
      navigation.goBack();
    } catch (err) {
      Alert.alert('Error', err.response?.data?.message || 'Failed to update challenge');
    } finally {
      setLoading(false);
    }
  };
