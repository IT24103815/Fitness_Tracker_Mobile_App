...
const CreateWorkoutScreen = ({ navigation }) => {
    ...
    const handleCreate = async () => {
        ...
        if (!titleTrimmed) {
            Alert.alert('Incomplete Form', 'Please provide a Title for this workout plan.');
            return;
        }

        if (form.goalType.length === 0) {
            Alert.alert('Incomplete Form', 'Please select at least one Goal Type for this plan.');
            return;
        }
        ...
        if (isNaN(durationNum) || durationNum < 1 || durationNum > 52) {
            Alert.alert('Invalid Duration', 'Plan duration must be between 1 and 52 weeks.');
            return;
        }
            ...
            >
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        />
                        <TouchableOpacity style={[styles.cancelBtn, { marginTop: 20 }]} onPress={() => setClientModalVisible(false)}>
                            <Text style={styles.cancelBtnText}>Cancel</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.saveBtn, { marginTop: 10, backgroundColor: '#EF4444' }]} onPress={() => { setForm({ ...form, client: '' }); setClientModalVisible(false); }}>
                            <Text style={styles.saveBtnText}>Clear Selection</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </Modal>
